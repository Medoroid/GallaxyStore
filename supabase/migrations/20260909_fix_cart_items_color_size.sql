-- Migration: Add color/size columns to cart_items and fix unique constraint
-- This migration must be applied to support variant option tracking

-- 1. Add color and size columns
...
-- 3. Add new unique constraint including color and size
-- This prevents duplicate items with same variant+color+size in same cart
ALTER TABLE cart_items
  ADD CONSTRAINT cart_items_cart_variant_color_size_unique
  UNIQUE (cart_id, variant_id, color, size);

-- 4. Update cart_items_detailed view to include color and size
